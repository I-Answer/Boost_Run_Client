...
public class Home_UI : MonoBehaviour {
    ...
    private IEnumerator Start() {
        ...
        curImage.sprite = spaceShipImage[UserManager.Player.NowSpaceShip].sprite;

        for (int i = 0; i < spaceShipImage.Length; i++) {
            if ((UserManager.Player.SpaceShipList & (1 << i)) != 0) {
    ...
    public void Select(int selectShip) {
        if (UserManager.Player.SelectNewSpaceShip(selectShip))
    ...
    private void CompleteBuy(Result[] updateShip) {
        completeBuy.Enable();
